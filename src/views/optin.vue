      ...
      <form @submit.prevent="onSubmit" class="flex flex-col gap-2 max-w-[600px]">
        <div v-for="(field, index) in formFields" :key="field" class="flex flex-col"
          ...
          <input :id="field" v-model="values[field]" :type="field === 'email' ? 'email' : 'text'"
            ...
            class="text-vw-dark" autocomplete="off" style="font-size: 0.45rem!important; padding: 6px;" />
        ...
        <div class="inline-block" ref="submitButtonRef" v-motion="submitButtonMotion">
          <button :disabled="isSaving" type="submit" class="bg-vw-light text-white font-medium py-1 px-4" style="min-height: 1rem!important; padding-top:0!important;padding-bottom:0!important;font-size: 0.85rem;display: block;margin: 0 auto;">
            Envoyer
          </button>
        </div>
      </form>
